...
def monthly_virtual_server_info():
    ...
    for item in item_dict: # Get inbound outbound bandwidth data #
        ...
        s = requests.Session();
        response = s.get(requestURL);
        item['inboundBandwidthUsage'] = round(1024 * float( json.loads(response.text).get('inboundPublicBandwidthUsage', '0') ), 2);
        item['outboundBandwidthUsage'] = round(1024 * float( json.loads(response.text).get('outboundPublicBandwidthUsage', '0') ), 2);
        item['totalBandwidthUsage'] = round(item['inboundBandwidthUsage']+item['outboundBandwidthUsage'], 2)
